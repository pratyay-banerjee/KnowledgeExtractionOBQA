...
from models.bert_qa_v2 import BertQAV2
...
def read_data_to_train(topk,ranked,knowledge,knowledgemap,use_gold_f2=False):
    ...
    for line in tqdm(gold,desc="Preparing Train Dataset:"):
        ...
        if use_gold_f2:
            premise = line[1]
            for index1,choiceA in enumerate(choices):
                for index2,choiceB in enumerate(choices):
                    if index1 == index2 or index1>index2:
                        continue
                    nidx = qid+":"+str(index1)+":"+str(index2)+":"+str(label)
                    nlabel = 2
                    if int(label) == index1:
                        nlabel = 0
                    elif int(label) == index2:
                        nlabel = 1
                    label_counts[nlabel]+=1
                    data[nidx]=Input(idx=nidx,passage=premise,a=choiceA,b=choiceB,c=NAN,label=nlabel)
                    if nlabel != 2:
                        label_counts[nlabel]+=1
                        data[nidx+"_2"]=Input(idx=nidx+"_2",passage=premise,a=choiceA,b=choiceB,c=NAN,label=nlabel)
        else:
...
for topkf1 in f1topklist:
    ...
    output_dir = "/scratch/pbanerj6/fp16-bertqav3-simple-f1-"+basedir+"-"+str(topkf1)+str(topkf2)+"-"+str(max_seq)+"/"
...
#                  num_of_epochs=epochs,action="train",fp16=True)
    ...
    model =  BertQAV2( output_dir=output_dir,topk=topk,
                 bert_model="bert-large-cased",do_lower_case=False,train_batch_size=32,seed=seed,learning_rate=1e-5,
                 eval_batch_size=64,max_seq_length=max_seq,num_labels=3,grad_acc_steps=1,
                 num_of_epochs=epochs,action="train",fp16=True)
