...
def read_examples(input_file):
    ...
    with open(input_file, "r", encoding='utf-8') as reader:
        while True:
            ...
            text_b = line[2]
            uuid = line[0]
            map_of_ids[unique_id]=uuid
            examples.append(
                InputExample(unique_id=unique_id, text_a=text_a, text_b=text_b))
            unique_id += 1
    return examples,map_of_ids
...
def main():
    ...
    with open(args.output_file, "w", encoding='utf-8') as writer:
        for input_ids, input_mask, example_indices in tqdm(eval_dataloader,"Extracting Features:"):
            ...
            for b, example_index in tqdm(enumerate(example_indices),desc="Per Batch:"):
...
#                         all_layers.append(layers)
                    ...
                    sum_of_layers = [
                            round(x.item(), 6) for x in sum_of_layers.detach().cpu().numpy()
                        ]
